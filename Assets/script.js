            ...
            fetch(weatherUrl)
                ...
                .then(function (current) {
                    ...
                    function display() {
                        ...
                        //for loop that gets every 8th index starting at 8 to get the next 5 days (there are 8 weather forecasts per day)

                        for (i = 8; i <= 40; i += 8) {
                            console.log(i);
                            console.log(current.list[i]);
                            let weatherIconCode = current.list[i].weather[0].icon;
                            let currentTemp = current.list[i].main.temp;
                            let currentHumid = current.list[i].main.humidity;
                            let currentWind = current.list[i].wind.speed;
                            let dt = current.list[i].dt;
                            // console.log(dt);     
                            // console.log(dayjs.unix(dt));       
                            let unixFormat = dayjs.unix(dt).format('dddd');

                            // Display the search results below the main content
                            let resultsContainer = document.getElementById('fiveDayWeather');
                            resultsContainer.classList.add("font-weight-bold", "col-12", "col-md-2", "col-lg-2", "m-1", "bg-light", "text-dark", 'border', 'border-dark', 'rounded');
                            resultsContainer.innerHTML = (unixFormat);
                            ...
                            weatherResults.innerHTML = (' Temp: ' + currentTemp + 'c ' + 'Humidity: ' + currentHumid + ' Wind Speed: ' + currentWind + 'kph');
